...
Nova provide a [puppet project](https://github.com/openstack/puppet-nova/)
who define how to run nova under an apache environment.
...
an async way by using gthread maybe in this case the parent thread consider that
...
There the used executor is an [`eventlet` executor](https://github.com/openstack/nova/blob/244c9240671d98b0df25b0ad0795b5de0c0c422c/nova/rpc.py#L226), so the initiated object will use eventlet and so the heartbeat thread will use a gthread.
